...
def get_augmentations(config):
    return Compose(parse_transforms(config.dataset.augmentations),
                   keypoint_params={'format': 'xy', "remove_invisible": False}, additional_targets={'target':'image'})


def get_transforms(config):
    return Compose(parse_transforms(config.dataset.transforms),
                   keypoint_params={'format': 'xy', "remove_invisible": False}, additional_targets={'target':'image'})