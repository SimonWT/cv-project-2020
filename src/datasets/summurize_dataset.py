...
def parse_json(filename):
    ...
    for i in range(0, landmarks.shape[0], 2):  # swap point if the righthand side point appears on the left from
...
def create_csv(root, tag='init'):
        ...
        if dirpath.endswith('ann'):
            ...
            for ann_name in filenames:
                ...
                if filehash not in hash_keys:
                    ...
                    df = df.append({'imgPath': img_name, 'annPath': os.path.join(dirpath, ann_name)}, ignore_index=True)
                else:
    ...
    df['isPathological'] = df['imgPath'].apply(lambda x: False if x.split(root)[1][1] == 'n' else True)
    ...
    for i in class_position:
        assert (df.loc[:, '_'.join([class_position[i], 'left', 'x'])]
                <
                df.loc[:, '_'.join([class_position[i], 'right', 'x'])]).all()
    return df
...
def split(csv):
    ...
    :param csv: path to the dataset annotation produced by 'create_csv'
    :return: Creates 3 csv files with train/val/test split. 60/20/20%
    """
    df = pd.read_csv(csv)
    print("Dataset length: {}".format(len(df)))
    train_idx, test_idx = train_test_split(np.arange(len(df)), shuffle=True, test_size=0.2, random_state=17,
                                           stratify=df.loc[:, 'isPathological'])
    train_idx, val_idx = train_test_split(train_idx, shuffle=True, test_size=0.25, random_state=17,
                                          stratify=df.loc[train_idx, 'isPathological'])
    print(len(train_idx), " :train, ", len(val_idx), ' :val, ', len(test_idx), ' :test')
    train_df = df.iloc[train_idx, :]
    val_df = df.iloc[val_idx, :]
    test_df = df.iloc[test_idx, :]
    train_df.to_csv(os.path.join('/home/semyon/cardiomethry/ChestXrayIndex/', 'train.csv'), index=False)
    val_df.to_csv(os.path.join('/home/semyon/cardiomethry/ChestXrayIndex/', 'val.csv'), index=False)
    test_df.to_csv(os.path.join('/home/semyon/cardiomethry/ChestXrayIndex/', 'test.csv'), index=False)
...
if __name__ == '__main__':
    ...
    split(os.path.join(root_folder, 'dataset.csv'))
